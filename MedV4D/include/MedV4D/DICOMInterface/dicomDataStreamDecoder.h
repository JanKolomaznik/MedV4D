...
public:
	...
	DicomDataStreamDecoder( 
		...
		bool dataSigned,
		uint16 *stream
		) : m_currBitsAvail(0),	m_buf( 0), m_bitsAllocated( (uint32)bitsAllocated), m_highBit( (uint32)highBit), 
		m_bitsStored( (uint32)bitsStored), m_isDataSigned( dataSigned), m_stream( stream )
	{
		ASSERT( m_stream );
		...
		D_PRINT( "DicomDataStreamDecoder: bitsAlloc: " << m_bitsAllocated
			<< " bitsStored: " << m_bitsStored 
			<< " highBit: " << m_highBit 
			<< " DataSigned: " << m_isDataSigned << std::endl
			);
	}
