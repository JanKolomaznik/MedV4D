...
template< typename TImage >
void
draw3DLine( TImage &aImage, typename TImage::Element aValue, Vector3f aStartPoint, Vector3f aEndPoint, float aWidth, Vector3f aPlaneNormal)
{
	BOOST_STATIC_ASSERT( TImage::Dimension == 3 );

	Vector3i radius;
	auto extents = aImage.GetElementExtents();
	for (int i = 0; i < 3; ++i) {
		radius[i] = std::ceil(aWidth / extents[i]);
	}

	Vector3i lMin, lMax, coord;
	auto startCoords = aImage.GetElementCoordsFromWorldCoords(aStartPoint);
	auto endCoords = aImage.GetElementCoordsFromWorldCoords(aEndPoint);

	lMin = M4D::minVect<int,3>(startCoords, endCoords);
	lMax = M4D::maxVect<int,3>(startCoords, endCoords);
	lMin -= radius;
	lMax += radius;
	lMin = M4D::maxVect<int,3>( lMin, aImage.GetMinimum() );
	lMax = M4D::minVect<int,3>( lMax, aImage.GetMaximum() );
	...
	for( coord[2] = lMin[2]; coord[2] <= lMax[2]; ++coord[2] ) {
		for( coord[1] = lMin[1]; coord[1] <= lMax[1]; ++coord[1] ) {
			for( coord[0] = lMin[0]; coord[0] <= lMax[0]; ++coord[0] ) {
				if (pointLineSegmentDistance(aStartPoint, aEndPoint, aImage.GetWorldCoordsFromElementCoords(coord)) < aWidth) {
					aImage.GetElement( coord ) = aValue;
				}
			}
		}
