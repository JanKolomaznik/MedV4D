...
public:
        ...
        void
        ReserveVertices ( size_t size ) {
                this->_points.reserve ( size );
        }

        void
        ReserveFaces ( size_t size ) {
                _faces.reserve ( size );
        }

        size_t
        AddVertex ( const VertexType & vertex ) {
                return this->_points.push_back ( vertex ), this->_points.size()-1;
        }

        size_t
        AddFace ( const FaceType & face ) {
                return _faces.push_back ( face ), _faces.size()-1;
        }
        ...
        void
        GetBoundingBox ( VectorType &firstCorner, VectorType &secondCorner ) const {
                _THROW_ ErrorHandling::ETODO();
        }
protected:
        RGBA_float	_color;
