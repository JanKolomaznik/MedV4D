		...
		)
{
	...
	if (aTransferFunction2D.eigenvalueProcessPrimary)
	{
		...
		float vesselness = computeVesselness(eigenvalues, aTransferFunction2D.primaryEigenvalueParameters)*primaryValuesMultiplier;
		value = (vesselness * range) + aMappedIntervalBands[0];
	}
	else
	{
	...
	if (aTransferFunction2D.eigenvalueProcessSecondary)
	{
		...
		float vesselness = computeVesselness(eigenvalues, aTransferFunction2D.secondaryEigenvalueParameters)*secondaryValuesMultiplier;
