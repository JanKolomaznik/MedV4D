...
bool
ADrawingMouseController::mouseMoveEvent(
		M4D::GUI::Viewer::BaseViewerState::Ptr aViewerState,
		const M4D::GUI::Viewer::MouseEventInfo &aEventInfo)
{
	...
	if ( state.viewType == M4D::GUI::Viewer::vt2DAlignedSlices && mIsDrawing) {
		...
		Vector3f diff;
		mTrackInfo.trackUpdate( aEventInfo.event->pos(), aEventInfo.event->globalPos(), Vector3f(glm::value_ptr(aEventInfo.point)), diff );

		drawStep( lastPos, Vector3f(glm::value_ptr(aEventInfo.point)), Vector3f(glm::value_ptr(aEventInfo.direction)));
...
{
	M4D::GUI::Viewer::ViewerState &state = *(boost::polymorphic_downcast< M4D::GUI::Viewer::ViewerState *>( aViewerState.get() ) );
	mTrackInfo.startTracking( aEventInfo.event->pos(), aEventInfo.event->globalPos(), Vector3f(glm::value_ptr(aEventInfo.point)));
