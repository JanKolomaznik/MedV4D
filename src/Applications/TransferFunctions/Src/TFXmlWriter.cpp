...
void TFXmlWriter::writeTestData(QIODevice *device){
	...
	TF::Color::MapPtr f = data->getColorMap();
...
void TFXmlWriter::writeFunction_(TFAbstractFunction::Ptr function){
	...
	writeStartElement("TransferFunction");
		...
		const TF::Color::MapPtr points = function->getColorMap();
		TF::Color::Map::const_iterator first = points->begin();
		TF::Color::Map::const_iterator end = points->end();
		for(TF::Color::Map::const_iterator it = first; it != end; ++it)
		{
				writePoint_(*it);	
		}
