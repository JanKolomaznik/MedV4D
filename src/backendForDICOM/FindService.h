...
public:
	 ...
	 */
	void FindForFilter(ResultSet &result, const string &patientForeName,
			const string &patientSureName, const string &patID,
	 ...
	 *  Returns resultSet container with table rows records.
	 */
	void FindStudiesAboutPatient(const string &patientID, ResultSet &result);
	 ...
	 */
	void FindWholeStudyInfo(const string &patientID, const string &studyID,
			StudyInfo &info);

	/**
	 *  The same as FindWholeStudyInfo, but find only serie IDs. Used when whole serie images are to be retrive later on
	 */
	void FindStudyInfo(const string &patientID, const string &studyID,
...
private:
	...
	void GetQuery(DcmDataset **query, const string *patientName,
			const string *patientID, const string *date,
			const string *referringMD, const string *description);
	// ...
	void GetStudyInfoQuery(DcmDataset **query, const string &patientID,
			const string &studyID);
	// ...
	void GetWholeStudyInfoQuery(DcmDataset **query, const string &patientID,
			const string &studyID);

	// supporting funcion common for all retrival functions
	void FindSupport(DcmDataset &queryDataSet, void *data,
			DIMSE_FindUserCallback callBack);

	// Supporting callbacks that cooperates with DCMTK functions ...
	static void TableRowCallback(void *callbackData, T_DIMSE_C_FindRQ *request,
			int responseCount, T_DIMSE_C_FindRSP *rsp,
			DcmDataset *responseIdentifiers);
	// ...
	static void WholeStudyInfoCallback(void *callbackData,
			T_DIMSE_C_FindRQ *request, int responseCount,
			T_DIMSE_C_FindRSP *rsp, DcmDataset *responseIdentifiers);
	// ...
	static void StudyInfoCallback(void *callbackData,
			T_DIMSE_C_FindRQ *request, int responseCount,
			T_DIMSE_C_FindRSP *rsp, DcmDataset *responseIdentifiers);
