...
TFAbstractFunction<dim>* TFCreator::createFunction_(){

	switch(structure_[mode_].function)
	{
		case TF::Types::FunctionRGBa:
		{
			return new TFRGBaFunction<dim>(dataStructure_);
		}
		case TF::Types::FunctionHSVa:
		{
			return new TFHSVaFunction<dim>(dataStructure_);
		}
	}

	tfAssert(!"Unknown function");
	return new TFRGBaFunction<dim>(dataStructure_);	//default
}
