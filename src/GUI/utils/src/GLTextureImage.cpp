...
CreateTextureFromImage( const M4D::Imaging::AImageRegion &image )
{
	...
	case 2:
		return CreateTextureFromImageTyped< 2 >( static_cast< const M4D::Imaging::AImageRegionDim<2> & >( image ) );
	case 3:
		return CreateTextureFromImageTyped< 3 >( static_cast< const M4D::Imaging::AImageRegionDim<3> & >( image ) );
	default:
