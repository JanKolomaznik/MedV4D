...
{  
  ...
  m_Conf.m_ConstantGradientValue = this->m_ConstantGradientValue;
}
///////////////////////////////////////////////////////////////////////////////
template<class TInputImage,class TFeatureImage, class TOutputPixelType>
MySegmtLevelSetFilter<TInputImage, TFeatureImage, TOutputPixelType>
...
#else
	  SetupGate();
	  m_Conf.m_UpdateBufferData = &this->m_UpdateBuffer[0];
...
//		  for(uint32 i=0; i<this->m_Layers.size() ; i++)
//		  {
//			  applyUpdateCalc.this->m_Layers[i] = (M4D::Cell::ApplyUpdateSPE::LayerType *) this->m_Layers[i].GetPointer();
//			  
//			  applyUpdateCalc.conf.layerBegins[i] = (M4D::Cell::SparseFieldLevelSetNode *) this->m_Layers[i]->Begin().GetPointer();
//			  applyUpdateCalc.conf.layerEnds[i] = (M4D::Cell::SparseFieldLevelSetNode *) this->m_Layers[i]->End().GetPointer();
//		  }
...
{	        
    ...
    //status image
    m_Conf.statusImageProps.imageData = (StatusType *)this->m_StatusImage->GetBufferPointer();
    m_Conf.statusImageProps.region = 
    	ConvertRegion<StatusImageType, M4D::Cell::TRegion>(*this->m_StatusImage);
    m_Conf.statusImageProps.spacing = ConvertIncompatibleVectors<M4D::Cell::TSpacing, typename StatusImageType::SpacingType>(this->m_StatusImage->GetSpacing());
...
::CalculateChange()
{
	this->AllocateUpdateBuffer();
	  ...
	  m_Conf.m_activeSetBegin = (M4D::Cell::SparseFieldLevelSetNode *) this->m_Layers[0]->Begin().GetPointer();
	  m_Conf.m_activeSetEnd = (M4D::Cell::SparseFieldLevelSetNode *) this->m_Layers[0]->End().GetPointer();
	  m_Conf.m_UpdateBufferData = &this->m_UpdateBuffer[0];
