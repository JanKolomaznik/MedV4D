...
Param --iso tells to use downloaded ISOs and where can be found for mounting them onto loopback device.
Param -a disables agreeing licences. Otherwise you have to write some 'yes' to agree.
Process begins with setting local YUM repositories pointing to the ISOs.
Then all default packages are installed with all their dependecies.
...
To install CellIDE run yumex, goto Group View$\rightarrow$Development$\rightarrow$Cell Development Tools.
...
And click 'Process Queue'. Note: you should have the ISOs mounted onto loopback devices.
...
The last part of develelopment environment is IBM Full-System Simulator (systemsym).
...
Be sure to download fedora 9 version of the simulator (cell-3.1-8.f9.*). Then instal it.
...
Now all neccessary stuff is installed.
...
It is ussually installed with Firefox3. There is following workaround:
\begin{enumerate}
...
\par
Another issue (stack issue) is with tcl (scripting language that is used for configuration of the systemsym).
There is bug with stack size chechking that causes cycling of tcl code.
To workeraound this problem you should use ulimit command that changes default environment of linux programs
...
Sysroot\_image is common binary image that can mounted and thus some additional content copyed into.
...
You have all the bash advantages like command and path completition available in contrast to 'text mode' of the IDE console.
...
Each folder has its own makefile that manages makefiles in its subfolders.
 So you can call only the top level one to build all projects in subfolders or any from the subfolders to build particular projects.
...
Next is neccassary to set up 'Build directory' to tell the IDE where shall search for makefile.
...
NOTE: when you turn oww netowrk manager, you have to turn on network service othervise the networking will not run properly.
For sevice management within ssh console /usr/sbin/ntsysv manager is quite useful.
